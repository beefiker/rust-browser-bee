...
fn render_background(commands: &mut DisplayList, layout_box: &LayoutBox) {
    get_color(layout_box, "background-color").map(|color| {
        commands.push(DisplayCommand::SolidRectangle(color, layout_box.dimensions.border_box()))
    });
}
...
fn render_borders(commands: &mut DisplayList, layout_box: &LayoutBox) {
    ...
    commands.push(
        DisplayCommand::SolidRectangle(color.clone(), Rectangle {
            x: border_box.x,
            y: border_box.y,
            width: d.border.left,
    ...
    commands.push(
        DisplayCommand::SolidRectangle(color, Rectangle {
            x: border_box.x,
